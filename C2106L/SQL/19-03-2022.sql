...
--Procedures = Function without return value1
...
BEGIN
	...
	WHERE Result.SubjectId = @SubjectId AND Result.Mark < 10;
	--NO RETURN !, but you can use "SELECT"
	SELECT * FROM Result;
END

EXECUTE up_IncreaseMark 3; 

UPDATE Result SET Mark = 6 WHERE StudentId = 2 AND SubjectId = 3;
SELECT * FROM Result;
...
ON Result AFTER INSERT AS
	...
	SET @Mark = (SELECT TOP 1 Mark FROM Result WHERE Mark < 0) 	
	IF @Mark < 0 
		BEGIN RAISERROR  ('Cannot update negative Marr',16,10); 
		ROLLBACK --undo
END
...
UPDATE Subject SET SubjectName = 'C Programming'
WHERE SubjectId = 1;
